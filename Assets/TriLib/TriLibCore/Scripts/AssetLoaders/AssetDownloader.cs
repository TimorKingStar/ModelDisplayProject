...
namespace TriLibCore
{
    ...
    public class AssetDownloader
    {
        ...
        /// <summary>异步地从给定的URI加载模型（接受zip文件） Loads a Model from the given URI Asynchronously (Accepts zip files).</summary>
        ...
        /// <param name="onLoad">当加载模型但资源可能仍处于挂起状态时调用主线程的方法。The Method to call on the Main Thread when the Model is loaded but resources may still pending.</param>
        /// <param name="onMaterialsLoad"> 在加载模型和资源时调用主线程的方法。The Method to call on the Main Thread when the Model and resources are loaded.</param>
        /// <param name="onProgress">当模型加载进度更改时要调用的方法。 The Method to call when the Model loading progress changes.</param>
        /// <param name="onError">发生任何错误时调用主线程的方法 The Method to call on the Main Thread when any error occurs.</param>
        /// <param name="wrapperGameObject">将作为加载的游戏对象的父对象的游戏对象。可以为null The Game Object that will be the parent of the loaded Game Object. Can be null.</param>
        /// <param name="assetLoaderOptions">加载模型时要使用的选项。 The options to use when loading the Model.</param>
        /// <param name="customContextData">将沿上下文传递的自定义数据 The Custom Data that will be passed along the Context.</param>
        /// <param name="fileExtension"> URI模型的扩展名或Zip文件中的模型   The extension of the URI Model or the Model inside the Zip file.</param>
        /// <param name="isZipFile">Pass <c>true</c> if your file is a Zip file.</param>
        /// <param name="haltTask">打开此字段以避免立即加载模型并将任务链接起来 Turn on this field to avoid loading the model immediately and chain the Tasks.</param>
